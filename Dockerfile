...
ARG AWS_CLI_VERSION=2.36.21
ARG AWS_SAM_CLI_VERSION=sam-cli-nightly
ARG AWS_SM_CLI_VERSION=1.2.835.0
...
RUN apt-get update && \
    apt-get -y --no-install-recommends install ca-certificates bash binutils curl tar unzip && \
...
# container as builder for preparing AWS cloud tools
FROM aws-cloud-tools-builder AS aws-cloud-tools-kpt-builder

LABEL stage="aws-cloud-tools-kpt-builder" \
      description="Debian-based container builder for preparing AWS cloud tool kpt CLI" \
      org.opencontainers.image.description="Debian-based container builder for preparing AWS cloud tool kpt CLI" \
      org.opencontainers.image.source=https://github.com/stefanbosak/aws-cloud-tools
...
# container as builder for preparing AWS cloud tools
FROM aws-cloud-tools-builder AS aws-cloud-tools-opentofu-builder

LABEL stage="aws-cloud-tools-opentofu-builder" \
      description="Debian-based container builder for preparing AWS cloud tool OpenTofu" \
      org.opencontainers.image.description="Debian-based container builder for preparing AWS cloud tool OpenTofu" \
      org.opencontainers.image.source=https://github.com/stefanbosak/aws-cloud-tools
...
FROM scratch AS aws-cloud-tools-binaries-aggregator

# transfer tools from builders
COPY --from=aws-cloud-tools-ansible-cli-builder "/usr/local/bin" "/usr/local/bin"
COPY --from=aws-cloud-tools-ansible-cli-builder "/usr/local/share/uv/tools" "/usr/local/share/uv/tools"
...
COPY --from=aws-cloud-tools-kpt-builder "/usr/local/bin" "/usr/local/bin"
